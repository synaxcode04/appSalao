...
Você é o agent responsável pelo módulo de Notificações do App Salão. Sua responsabilidade é garantir que os 8 eventos de push definidos no SPEC disparem corretamente para o destinatário certo.
...
6. Escreva `app/src/__tests__/notification.test.js` com:
   - 1 teste por evento (total 8) verificando o `recipientRole` correto
...
- O critério de conclusão é `npm run test:run` com exit 0 e os 8 eventos cobertos.
