#!/bin/bash
# PostToolUse: roda testes após edição em arquivos de autenticação e controle de licença
INPUT=$(cat)
...
if [[ "$TOOL" == "Edit" || "$TOOL" == "Write" ]]; then
  if echo "$FILE" | grep -qiE '(ProtectedRoute|SuspendedScreen|OwnerLayout|SalonDetails|__tests__)'; then
    echo "" >&2
    echo "▶ [auth-guard] Rodando testes após edição em: $FILE" >&2
    ...
    if [ $TEST_EXIT -ne 0 ]; then
      echo "⚠️  [auth-guard] Testes falhando após edição. Corrija antes de continuar." >&2
    else
      echo "✅ [auth-guard] Todos os testes passando." >&2
    fi
  fi
fi
