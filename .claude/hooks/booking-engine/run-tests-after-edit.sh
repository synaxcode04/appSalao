#!/bin/bash
# PostToolUse: roda testes do BookingEngine após cada edição em arquivo de escopo
INPUT=$(cat)
...
if [[ "$TOOL" == "Edit" || "$TOOL" == "Write" ]]; then
  if echo "$FILE" | grep -qiE '(BookingEngine|__tests__)'; then
    echo "" >&2
    echo "▶ [booking-engine] Rodando testes após edição em: $FILE" >&2

    # Navega para a raiz do projeto e entra em app/
    ...
    if [ $TEST_EXIT -ne 0 ]; then
      echo "⚠️  [booking-engine] Testes falhando após edição. Corrija os erros antes de continuar." >&2
    else
      echo "✅ [booking-engine] Todos os testes passando." >&2
    fi
  fi
fi
