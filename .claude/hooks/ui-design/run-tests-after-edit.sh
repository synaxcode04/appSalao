#!/bin/bash
# PostToolUse: roda testes do módulo cliente após cada edição em arquivo de escopo do ui-design
INPUT=$(cat)
...
if [[ "$TOOL" == "Edit" || "$TOOL" == "Write" ]]; then
  if echo "$FILE" | grep -qiE '(pages[\\/]client|SalonLayout|BookingEngine|ClientIdentityForm|__tests__)'; then
    echo "" >&2
    echo "▶ [ui-design] Rodando testes após edição em: $FILE" >&2
    ...
    if [ $TEST_EXIT -ne 0 ]; then
      echo "⚠️  [ui-design] Testes falhando após edição. Corrija os erros antes de continuar." >&2
    else
      echo "✅ [ui-design] Todos os testes passando." >&2
    fi
  fi
fi
