...
- **Nunca** dar `git push` (pra `dev` ou `main`) fora desses dois slash commands, mesmo após aprovação do revisor de código — o commit local continua automático, mas o push fica sob controle do usuário.
...
- Lembrete de agendamento do dia via push (registrado em 2026-08-04, plano detalhado em 2026-08-09): notificar o cliente automaticamente quando virar o dia do agendamento (ex: "você tem um horário hoje às 14h"). Diferente do item de WhatsApp acima — seria push via OneSignal (canal já existente no app), não WhatsApp.
  - **Correção de contagem:** seria o **8º evento** em `notify.js` (o mapa tem 7 hoje, não 8 — a menção antiga a "9º" estava errada).
  - **Partes de baixo/médio risco:** novo evento no `EVENT_MAP` de `notify.js` (trivial); endpoint que busca agendamentos `scheduled` do dia e dispara os pushes — precisa rodar via `service_role` (cliente não tem Supabase Auth) e calcular "hoje" no fuso do Brasil, não UTC (mesma classe de bug já corrigida em `list_by_client`, commit 687538b).
  - **Parte de alto risco — o agendador:** Vercel Cron exigiria criar um `vercel.json` que o projeto não tem hoje, o que colide diretamente com o guardrail de "não criar estrutura nova" (já causou o incidente de 404 geral em produção em 2026-08-01) e com a pendência ainda não resolvida do Root Directory da Vercel. **Alternativa recomendada: `pg_cron`/`pg_net` do Supabase**, que dispara o HTTP de dentro do banco sem tocar em `vercel.json` — risco bem menor para este projeto.
  - **Pontos de decisão em aberto:** agendador (`pg_cron` recomendado vs Vercel Cron vs cron externo); horário/fuso exato do disparo (sugestão: 06:00 America/Sao_Paulo); idempotência (tabela de log para evitar push duplicado no mesmo dia); comportamento se o cliente cancelar depois do lembrete já ter saído; cliente sem opt-in de push (pular silenciosamente, assumido).
  - **Status:** não implementar antes do beta com o cliente piloto (2026-08-10). Retomar depois, começando pela Fase 0 (decisão do agendador) antes de qualquer código.
